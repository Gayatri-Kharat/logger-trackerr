...
import { Radar, Lock, Globe, Activity, ChevronRight, Server, Link2, UserCircle, CheckCircle2, Search, Database, AlertCircle, Users, Key } from 'lucide-react';
import { SAVED_PROFILES, DEMO_SERVICES, ENVIRONMENTS, CLUSTERS, API_PATHS, API_URL_TEMPLATE, AUTH_URL_TEMPLATE, DEFAULT_REALM } from '../constants';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  // Auth Mode: User (Password Grant) vs Service (Client Credentials)
  ...
  // Token state to hold the token after successful scan/auth
  ...
  useEffect(() => {
    ...
    let authBase = AUTH_URL_TEMPLATE;
    authBase = authBase.replace('{cluster}', selectedClusterId);
    authBase = authBase.replace('{env}', selectedEnvId);
    // Remove trailing slash if present
    authBase = authBase.replace(/\/$/, '');

    // Append the Token Path (Resolving Realm)
  ...
  const handleProfileChange = (profileId: string) => {
    ...
    if (profile) {
      ...
      // Heuristic: If label contains 'User', assume User Login, otherwise maybe Service?
      setIsUserLogin(true); 
    }
  };
  ...
  const handleScanEnvironment = async (e: React.FormEvent) => {
    ...
    let tokenToUse = password; // Default fallback (Direct Access Token)

    try {
        // 1. Attempt Authentication First (Keycloak)
        // We do this to get a real Access Token instead of sending the password/secret directly
        try {
            console.log("Attempting to fetch token from:", constructedAuthUrl);
            const authResponse = await authenticate(
                constructedAuthUrl,
                username,
                password,
                { isUserLogin, publicClientId }
            );
            if (authResponse.accessToken) {
                tokenToUse = authResponse.accessToken;
                setAuthToken(tokenToUse);
                console.log("Authentication successful, proceeding to discovery...");
            }
        } catch (authErr) {
            console.warn("Authentication failed, falling back to direct credentials:", authErr);
            // We proceed with 'password' as the token, in case it's a pre-generated token
        }

        // 2. Perform Discovery with the Token
  ...
  const handleFinalConnect = () => {
    ...
    // If we have a valid auth token from step 1, we should pass it implicitly 
    // or rely on the App to re-authenticate. 
    // Since App.tsx calls handleLogin which calls authenticate again, 
    // we just pass the credentials and endpoints as before.
    // However, to avoid double-auth in App.tsx (which might be redundant), 
    // ideally we would pass the token, but the interface expects username/password.
    // The App.tsx logic handles re-auth, which is fine for security.
            ...
            <form onSubmit={handleScanEnvironment} className="space-y-8 animate-in slide-in-from-right-4 fade-in duration-300 flex-1 flex flex-col">
              ...
              <div className="space-y-4">
                 <div className="flex items-center justify-between">
                    ...
                    {/* AUTH MODE TOGGLE */}
                    <div className="flex bg-[#0a0c10] p-0.5 rounded-lg border border-slate-800">
                        <button
                          type="button"
                          onClick={() => setIsUserLogin(true)}
                          className={`flex items-center gap-1 px-3 py-1 rounded-md text-[9px] font-bold uppercase transition-all ${isUserLogin ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-600 uppercase ml-1">
                            {isUserLogin ? 'Username / Principal' : 'Service Client ID'}
                        </label>
                        <input
                            type="text"
                            value={username}
                    ...
                    <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-600 uppercase ml-1">
                            {isUserLogin ? 'Password' : 'Client Secret'}
                        </label>
                 ...
                 {/* ADVANCED: Public Client ID (Hidden unless User Mode) */}
                 {isUserLogin && (
                    <div className="space-y-1 pt-2">
                        <label className="text-[9px] font-bold text-indigo-400 uppercase ml-1 flex items-center gap-1">
                            Auth Client ID (Public)
                        </label>
                        <input
                            type="text"
                            value={publicClientId}
                            onChange={(e) => setPublicClientId(e.target.value)}
                            placeholder="e.g. apigw"
                            className="w-full bg-[#13161c] border border-indigo-900/30 rounded-xl px-3 py-2 text-indigo-300 text-xs font-mono focus:border-indigo-500 outline-none"
                        />
                    </div>
                 )}
              </div>
